...
SELECT FIRST_NAME + ' ' + LAST_NAME AS 'namd'
     , EMPLOYEE_ID
     , HIRE_DATE
  FROM employees
...
/* 6. 그병와 수당율에 대한 지출보고서. 수당을 받는 모든 사원의 이름, 급여, 임무
      수당율을 출력(35개 행) */
...
/* 7. 사원의 성이 s로 끝나는 사원의이름과 업무를 아래와 같이 출력(18개 행)
...
-- 서브쿼리
/* 사원의 급여 정보중 최소 급여를 받는 사원의 이름을, 업무, 급여, 입사일 출력(21개행) */
...
SELECT DEPARTMENT_ID
     ...
     , '$' + FORMAT(SUM(SALARY), '#,#') AS 'Salary SUM'
  FROM employees
 GROUP BY DEPARTMENT_ID, JOB_ID, JOB_ID WITH ROLLUP;
